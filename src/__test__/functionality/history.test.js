...
describe('GameHistory Class', () => {
  ...
  test('It should use LS to store history', () => {
    ...
    expect(GH.getHistory()[0]).toEqual(
      expect.objectContaining({ winner: baseHistory[0] })
    )
  ...
  test('It should return the MaxScore', () => {
    ...
    expect(GH.getTopScores(GH.getHistory())[0]).toEqual(
      expect.objectContaining({ winner: 'WINNER' })
    )
  })
})
